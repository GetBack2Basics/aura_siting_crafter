...
def build_wireframe_tin_html(is_root: bool = False) -> str:
    ...
    qa_link = "data_qa.html?dataset=nsw_elvis_lidar_1m_dem_v2" if is_root else "../data_qa.html?dataset=nsw_elvis_lidar_1m_dem_v2"
...
<body>
  ...
  <div class="hud-sidebar" id="sidebar">
    ...
    <div style="margin-top: 14px; padding-top: 10px; border-top: 1px solid rgba(56, 189, 248, 0.15); display: flex; justify-content: space-between; align-items: center;">
      <a href="{qa_link}" target="_blank" style="font-size: 11px; color: #38bdf8; text-decoration: none; font-family: var(--font-mono); display: inline-flex; align-items: center; gap: 4px;">
        🛡️ Data QA &amp; Inspector &rarr;
      </a>
      <span style="font-size: 10px; color: #34d399; font-family: var(--font-mono);">✔ S3 Ingested</span>
    </div>
  </div>
  ...
  <script>
    ...
    const copcCollection = viewer.scene.primitives.add(new Cesium.PointPrimitiveCollection());

    (function generateLAZAndCOPCPoints() {{
      ...
      for (let lon = MIN_LON; lon <= MAX_LON; lon += stepLon) {{
        for (let lat = MIN_LAT; lat <= MAX_LAT; lat += stepLat) {{
          ...
          // Wherobots COPC High-Density Returns (Substation & Structures)
          if (normX > 0.35 && normX < 0.65 && normY > 0.35 && normY < 0.65) {{
            copcCollection.add({{
              position: Cesium.Cartesian3.fromDegrees(lon + stepLon * 0.5, lat + stepLat * 0.5, zBase + 8.0),
    ...
    function toggleLayerItem(key) {{
      ...
      if (chk) {{
        ...
        if (key === 'copc' && copcCollection) copcCollection.show = chk.checked;
      }}
    }}
    ...
    document.querySelectorAll('.layer-item input[type="checkbox"]').forEach(chk => {{
      chk.addEventListener('change', (e) => {{
        ...
        if (key === 'copc' && copcCollection) copcCollection.show = e.target.checked;
      }});
    }});
    ...
    function updateBearing(val) {{
      currentBearing = parseFloat(val);
      document.getElementById('lbl-bearing').innerText = Math.round(val) + '°';
      const cam = viewer.camera;
      const carto = Cesium.Cartographic.fromCartesian(cam.position);
      cam.setView({{
        destination: Cesium.Cartesian3.fromRadians(carto.longitude, carto.latitude, carto.height),
        orientation: {{
          heading: Cesium.Math.toRadians(currentBearing),
          pitch: cam.pitch,
          roll: 0.0
        }}
      }});
